...
import { LayoutDashboard, Users, UserCog, Wrench, ClipboardList, LogOut, Truck, Map, AlertOctagon } from 'lucide-react';
...
const SidebarAdmin = () => {
  ...
  const navItems = [
    ...
    { name: 'Nhân viên', path: '/admin/staff', icon: UserCog },
    { name: 'Dịch vụ', path: '/admin/services', icon: Wrench },
    { name: 'Đơn cứu hộ', path: '/admin/requests', icon: ClipboardList },
    { name: 'Khiếu nại', path: '/admin/complaints', icon: AlertOctagon },
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-64 bg-white border-r border-[#E2E8F0] flex flex-col z-40">
      ...
      <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        <p className="px-3 text-xs font-semibold text-[#94A3B8] uppercase tracking-wider mb-4">Quản trị</p>
        {navItems.map((item) => {
