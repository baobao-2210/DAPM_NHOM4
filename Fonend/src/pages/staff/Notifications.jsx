...
export default function Notifications() {
  ...
  const { data: notifications = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axiosClient.get(`/ThongBao/tai-khoan/${idTaiKhoan}`);
      ...
      ).sort((a, b) => new Date(b.ngayTao || b.thoiGian).getTime() - new Date(a.ngayTao || a.thoiGian).getTime());
    },
  ...
  const markAsReadMutation = useMutation({
    mutationFn: (idThongBao) => axiosClient.put(`/ThongBao/${idThongBao}/read`),
  ...
  const handleMarkAsRead = (id, trangThai) => {
    if (trangThai === 'ChuaDoc') markAsReadMutation.mutate(id);
  };

  const displayNotifs = filter === 'unread' ? notifications.filter(n => n.trangThai === 'ChuaDoc') : notifications;
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => markAllAsReadMutation.mutate()} disabled={markAllAsReadMutation.isPending} className="px-5 py-2.5 bg-white border border-gray-200 text-blue-600 font-bold rounded-xl shadow-sm hover:bg-blue-50 transition-colors">
          Đánh dấu tất cả đã đọc
        </button>
      </div>

      <div className="flex space-x-2 border-b border-gray-200 mb-6">
        <button onClick={() => setFilter('all')} className={`px-6 py-3 text-sm font-bold border-b-2 transition-colors ${filter === 'all' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-800'}`}>Tất cả</button>
        <button onClick={() => setFilter('unread')} className={`px-6 py-3 text-sm font-bold border-b-2 transition-colors ${filter === 'unread' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-800'}`}>
          Chưa đọc ({notifications.filter(n => n.trangThai === 'ChuaDoc').length})
        </button>
      </div>
        ...
        {displayNotifs.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {displayNotifs.map(notif => (
              <div key={notif.id} onClick={() => handleMarkAsRead(notif.id, notif.trangThai)} className={`p-6 flex gap-4 cursor-pointer transition-colors ${notif.trangThai === 'ChuaDoc' ? 'bg-blue-50/20 hover:bg-blue-50/50' : 'hover:bg-gray-50'}`}>
